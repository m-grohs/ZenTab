...
import { applySearchSettings, SpeechRecognition } from './search.js';
...
function syncForm(s) {
	...
	setChecked('s-show-search', showSearch);
	showEl('row-voice-toggle', showSearch && !!SpeechRecognition);
	showEl('search-engine-section', showSearch);
	setChecked('s-show-voice', s.showVoice !== false);

	setInputValue('s-search-engine', s.searchEngine || 'google');
	const isCustom = s.searchEngine === 'custom';
	showEl('s-search-url', showSearch && isCustom);
	showEl('s-search-url-hint', showSearch && isCustom);
	setInputValue('s-search-url', s.searchUrl || '');

	setInputValue('s-search-width', s.searchBarWidth ?? 520);
	setInputValue('s-search-height', s.searchBarHeight ?? 40);
	setInputValue('s-search-font-size', s.searchBarFontSize ?? 14);
	setInputValue('s-search-gap-top', s.searchGapTop ?? 0);
	setInputValue('s-search-gap-bottom', s.searchGapBottom ?? 0);

	// Search bg colour
	const sbg = s.searchBarBg || '';
	setInputValue('s-search-bg-text', sbg);
	setInputValue('s-search-bg-pick', sbg || '#1a1a1f');

	// Search text colour (null = inherit global)
	const stc = s.searchBarTextColor || '';
	setInputValue('s-search-text-text', stc);
	setInputValue('s-search-text-pick', stc || '#e8e8f0');
}
...
function wireNumber(id, field, rerender = false) {
	document.getElementById(id)?.addEventListener('input', (e) => setSetting(field, +e.target.value, rerender));
}
...
export function initSettings() {
	...
	wireNumber('s-rows', 'rows', true); // changes DOM row count
	wireNumber('s-per-row', 'perRow', true); // changes DOM link count
	...
	// ── Search ──
	wireToggle('s-show-search', 'showSearch', (shown) => {
		showEl('search-engine-section', shown);
		if (SpeechRecognition) showEl('row-voice-toggle', shown);
		const isCustom = state.settings.searchEngine === 'custom';
		showEl('s-search-url', shown && isCustom);
		showEl('s-search-url-hint', shown && isCustom);
	});

	if (SpeechRecognition) wireToggle('s-show-voice', 'showVoice');

	document.getElementById('s-search-engine')?.addEventListener('change', (e) => {
		const v = e.target.value;
		const isCustom = v === 'custom';
		showEl('s-search-url', isCustom);
		showEl('s-search-url-hint', isCustom);
		if (isCustom) document.getElementById('s-search-url')?.focus();
		setSetting('searchEngine', v);
	});

	document.getElementById('s-search-url')?.addEventListener('input', (e) => setSetting('searchUrl', e.target.value.trim()));

	wireNumber('s-search-width', 'searchBarWidth');
	wireNumber('s-search-height', 'searchBarHeight');
	wireNumber('s-search-font-size', 'searchBarFontSize');
	wireNumber('s-search-gap-top', 'searchGapTop');
	wireNumber('s-search-gap-bottom', 'searchGapBottom');

	wireColor('s-search-bg-text', 's-search-bg-pick', 'searchBarBg');
	wireColor('s-search-text-text', 's-search-text-pick', 'searchBarTextColor');
